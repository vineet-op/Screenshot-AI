...
export default function All_Images() {
  ...
  const getAllScreenshots = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/user/getall_images`,
        {
      ...
      toast.error("Failed to load screenshots. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="w-screen lg:px-60 px-10 min-h-screen bg-black/95 relative overflow-hidden text-white">
      ...
      <Link
        href={"/dashboard"}
        className="absolute top-4 left-4 z-10 flex items-center gap-2 hover:scale-102 transition-transform duration-300"
      >
        <div className="absolute p-2 min-w-44 top-4 left-4 flex items-center gap-2 text-white hover:text-blue-400 transition-colors">
          <ArrowLeft className="w-5 h-5" />
          <span className="text-sm font-semibold">Back to Dashboard</span>
        </div>
      </Link>
      <div className="flex items-center h-full flex-col text-center space-y-8 mt-24 md:px-0">
        <h1 className="text-white text-5xl font-sans relative font-semibold tracking-tighter">
          All Screenshots
        </h1>
          ...
          {loading ? (
            ...
            screenshots.map((screenshot) => (
              <motion.div
                ...
                transition={{
                  duration: 0.3,
                  delay: screenshot._id
                    ? screenshots.findIndex((s) => s._id === screenshot._id) *
                      0.1
                    : 0,
                }}
      ...
      <AnimatePresence mode="wait">
        {selectedId && selectedCard && (
          <motion.div
            ...
            className="fixed inset-0 backdrop-blur-lg flex items-start justify-center font-sans z-50 overflow-y-auto py-8 px-4"
            onClick={(e) => {
          ...
          >
            <div className="w-full max-w-xl my-auto">
              <Card
                ...
                className="w-full bg-gradient-to-br from-blue-100 to-white border-blue-200 shadow-xl hover:shadow-2xl transition-all duration-300"
              >
                <CardHeader>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                          {selectedCard.mimetype} •{" "}
                          {formatFileSize(selectedCard.size)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <div
                        className={`flex items-center gap-2 px-3 py-1 rounded-full border ${getStatusColor(selectedCard.status)}`}
                      >
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="relative group">
                    <div className="aspect-video bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700 rounded-xl overflow-hidden shadow-inner">
                      ...
                      <img
                        ...
                        className={`w-full h-full object-contain transition-all duration-300 ${imageLoaded ? "opacity-100" : "opacity-0"} group-hover:scale-105`}
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                      ...
                      <div className="max-h-40 overflow-y-auto">
                        <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                          {showFullText
                            ? selectedCard.extractedText
                            : truncateText(selectedCard.extractedText, 150)}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                      ...
                      <p className="text-sm text-slate-600 dark:text-slate-300">
                        {formatDate(selectedCard.uploadedAt)}
                      </p>
                    </div>
                  </div>
